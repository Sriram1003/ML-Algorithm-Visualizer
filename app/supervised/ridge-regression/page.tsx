...
import { RidgeRegression, trainTestSplit, calculateRegressionMetrics, normalize } from '@/lib/ml';

// Define the structure of the metrics returned after training
interface RidgeRegressionMetrics {
  accuracy?: number; // Not applicable for regression, keeping for UI consistency if needed or removing
  mse: number; // Mean Squared Error
  rmse: number;
  r2: number; // R-squared
}

const RidgeRegressionPage: React.FC = () => {
  const [data, setData] = useState<any[][] | null>(null);
  const [isTraining, setIsTraining] = useState<boolean>(false);
  const [metrics, setMetrics] = useState<RidgeRegressionMetrics | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [alpha, setAlpha] = useState<number>(1.0); // Regularization parameter
  ...
  const trainModel = () => {
      ...
      setTimeout(() => {
          try {
              ...
              ridge.fit(X_train as number[][], y_train as number[], alpha, 0.01, 1000);
              const predictions = ridge.predict(X_test as number[][]);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-purple-400">
            Interactive Ridge Regression
          </h1>
          <div className="bg-black p-6 rounded-lg border border-purple-500">
            <h2 className="text-2xl font-semibold text-purple-400 mb-4">How Ridge Regression Works</h2>
            <p className="text-gray-300 mb-4">
              Ridge Regression is a type of linear regression that includes a regularization term to prevent overfitting. It adds a penalty equal to the square of the magnitude of coefficients.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-purple-700 p-4 rounded-lg">
                <h3 className="font-semibold text-purple-400 mb-2">1. Regularization</h3>
                <p className="text-sm text-gray-200">Helps to reduce model complexity and prevent overfitting.</p>
              </div>
              <div className="bg-purple-700 p-4 rounded-lg">
                <h3 className="font-semibold text-purple-400 mb-2">2. Cost Function</h3>
                <p className="text-sm text-gray-200">Minimizes the sum of squared residuals plus the penalty term.</p>
              </div>
              <div className="bg-purple-700 p-4 rounded-lg">
                <h3 className="font-semibold text-purple-400 mb-2">3. Hyperparameter Tuning</h3>
                <p className="text-sm text-gray-200">The regularization parameter (alpha) controls the strength of the penalty.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <Card className="bg-black border-purple-500 border">
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm mb-2 text-gray-200">Regularization Parameter (α) ({alpha})</label>
                  <Slider
                    value={[alpha]}
...
export default RidgeRegressionPage;